<template>
  <div>
    <!-- 顶部导航 -->
    <!-- <van-row class="topsearchNav">
      <span class="iconfont icon-K"></span>
      <div class="nav-lit">
        <router-link tag="h3" to="/choiceness/choicenessmale" :class="['nav-item', { active: sexType === 'male' }]" @click="changSex('male')">男生</router-link>

        <router-link tag="h3" to="/choiceness/choicenessfemale" :class="['nav-item', { active: sexType === 'female' }]" @click="changSex('female')">女生</router-link>
      </div>
      <a href="/search" class="iconfont icon-sousuo"></a>
    </van-row>-->

    <!-- 第二种方法 组件 -->

    <headerBar :headerTitle="headerTitle" :sex="sex" v-on:change-sex="changeSex"></headerBar>
    <div class="featured-book-list" ref="featuredBookList">
      <!-- 轮播图 -->
      <van-swipe :autoplay="3000">
        <van-swipe-item v-for="(image, index) in imgs" :key="index">
          <img :src="image" alt />
        </van-swipe-item>
      </van-swipe>

     <van-grid square :border="false">
        <van-grid-item to="/classify"  icon="https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=2039795939,669276753&fm=26&gp=0.jpg" text="分类" />
       
        <van-grid-item to="/bookshelf" icon="data:image/png;base64," text="书架" />
        <van-grid-item to="/vip" icon="data:image/png;base64," text="VIP" />
        <van-grid-item to="/recharge" icon="data:image/png;base64," text="充值" />
      </van-grid>

      <!-- 标题 -->
      <div class="book-list-section" v-for="module in modules" :key="module._id">
        <div class="book-list-top">
          <h2 class="book-list-title">{{ module.title }}</h2>
          <div class="book-list-more fr">
            <router-link :to="{ name: '更多详情', params: {id : module._id} }" class="red">更多</router-link>
          </div>
        </div>
        <homeList :book-info="{ id: module._id }" @load-result="loadResult"></homeList>
      </div>

      
    </div>
  </div>
</template>

<script>
import { mapMutations } from "vuex";
import api from "../fetch/api";
import { FEATURED_PAGE } from "../util/util";
import headerBar from "../components/header/headerBar";
import homeList from "../components/choiceness/homeList";
export default {
  data() {
    return {
      isShowPageLoading: true,
      sex: "male", // 默认为男生
      modules: [],
      loadModules: [],
      headerTitle: "",
      imgs: [
        "https://plf-new.zhuishushenqi.com/management/images/20200529/f2edc16ef2e4490cb95a8737c4c7e39d.jpg",
        "https://plf-new.zhuishushenqi.com/management/images/20200529/8961f306741d4a6da4854e1b70398c6e.jpg",
        "https://plf-new.zhuishushenqi.com/management/images/20200529/f5cff6ff56204e1bafd3ee7e13ccaea9.jpg",
        "https://plf-new.zhuishushenqi.com/management/images/20200522/37f7369d766f414a91123dc886caad33.jpg"
      ]
    };
  },
  watch: {
    loadModules: function() {
      if (this.loadModules.length === 0) {
        this.isShowPageLoading = false;
      }
    }
  },
  created: function() {
    this.SET_HEADER_INFO({
      title: "梦远书城",
      type: FEATURED_PAGE
    });
    this.fetchData();
  },
  methods: {
    ...mapMutations(["SET_HEADER_INFO"]),
    fetchData: function() {
      api.getFeaturedData().then(data => {
        /*
         * 通过开发工具获取到的接口，无排序和男/女分类，其他地方也没找到相关的接口，
         * 所以此处只好获取数据之后再进一步处理。
         */
        data = Array.from(data).sort((a, b) => {
          return a.order - b.order;
        });
        let sexOrder = this.sex === "male" ? [ 7,5,9,4,6,8] : [1, 4, 6, 8,7,5];
        data = data.filter(obj => {
          return sexOrder.includes(obj.order) && obj.type === 0;
        
        });
        this.modules = data;
        this.loadModules = Array.from(data, value => value._id);
      });
    },
    changeSex: function(sex) {
      this.isShowPageLoading = true;
      document.body.scrollTop = 0;
      this.sex = sex;
      this.fetchData();
    },
    loadResult: function(id) {
      this.loadModules.splice(this.loadModules.indexOf(id), 1);
    }
  },
  components: {
    headerBar,
    homeList
  }
};
</script>

<style lang="scss" scoped>
.featured-book-list {
  margin: 40px 0 60px;
}

.swiper-container {
  width: 100%;
  height: 100%;
}
.swiper-slide {
  text-align: center;
  font-size: 18px;
  background: #fff;

  /* Center slide text vertically */
  display: -webkit-box;
  display: -ms-flexbox;
  display: -webkit-flex;
  display: flex;
  -webkit-box-pack: center;
  -ms-flex-pack: center;
  -webkit-justify-content: center;
  justify-content: center;
  -webkit-box-align: center;
  -ms-flex-align: center;
  -webkit-align-items: center;
  align-items: center;
}
.book-list-section {
  position: relative; //margin-bottom: 10px;
  border-bottom: 10px solid #f6f7f9;
  padding: 15px 0 0 15px;
  background: #fff;
}




.book-list-top {
  position: relative;
  margin-bottom: 5px;
  overflow: hidden;

  .book-list-title {
    font-weight: 400;
    display: inline-block;
    color: #33373d;
    line-height: 1;
    border-left: 2px solid #ed424b;
    padding-left: 8px;
  }
  .book-list-more {
    display: inline-block;

    a {
      position: absolute;
      top: 0px;
      right: 15px;
      font-size: 13px;

      &::after {
        content: ">";
        display: inline-block;
        margin-left: 3px;
      }
    }
  }
}
</style>