  ...
  <div class="shelf-list">
    <ul class="shelf-book-list" v-if="shelfBookList.length > 0">
      <li v-for="book in shelfBookList" :key="book.id" class="shelf-book-item">
        <router-link :to="{ name: '书籍详情', params: { id: book.id }}">
          ...
          <div class="book-info">
            ...
            <p class="meta author">{{ book.author }}&nbsp;&nbsp;著</p>
            <p class="meta">连载至：&nbsp;&nbsp;{{ book.lastChapter }}</p>
          </div>
        </router-link>
        <span class="delete" @click="deleteBook(book.id)">
          <span class="iconfont icon-shanchu4 icon"></span>
        </span>
      </li>
    </ul>
...
import dialog from '../../components/loading/diaolog';
